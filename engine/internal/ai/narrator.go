...
// NarrationDisclaimer is the honesty notice the engine attaches to every
// narration, live or stub. It is engine-authored (never model-authored) so the
// caveat cannot be paraphrased away or dropped by the model: the narrative is
// AI-generated and an analyst must verify it against the underlying evidence
// before acting. Naming Gemini keeps the provenance of the prose explicit.
const NarrationDisclaimer = "AI-generated by Google Gemini from the engine's reconstructed chain. Treat findings and hypotheses as leads, not facts, and verify against the source events before taking action."
