...
// classifySource maps an honest source label to a provenance family so the feed
// can visually separate "we asked Splunk's own data / AI" from "we cross-checked
// an external catalog". Matching is on substrings of the engine-supplied label,
// so renamed indexes (e.g. "Splunk winsec index") still classify correctly. The
// label text itself is never rewritten: what the engine reports is what shows.
type SourceKind = 'splunk' | 'external' | 'mitre' | 'other'

export function classifySource(source: string | undefined): SourceKind {
  if (!source) return 'other'
  const s = source.toLowerCase()
  if (s.includes('splunk') || s.includes('saia')) return 'splunk'
  if (s.includes('mitre') || s.includes('att&ck') || s.includes('attack')) return 'mitre'
  if (s.includes('kev') || s.includes('nvd') || s.includes('cve') || s.includes('cisa')) return 'external'
  return 'other'
}
...
export function InvestigationTracePanel({ activity, busy, hasChain, onNavigate }: InvestigationTracePanelProps) {
  ...
  // Distinct honest source labels, in first-seen order, for the legend strip.
  ...
  return (
    <div className="trace-panel">
      <p className="trace-disclaimer">
        <span className="trace-disclaimer-tag">AI generated</span>
        Live tool calls from the narrator and threat-intel agents. Verify findings before acting.
      </p>
      {steps.length > 0 && (
      ...
      {sources.length > 0 && (
        <div className="trace-legend" aria-label="Sources consulted">
          {sources.map((src) => {
            const kind = classifySource(src)
            return (
              <span key={src} className={`trace-legend-chip trace-src-${kind}`} title={SOURCE_KIND_LABEL[kind]}>
                {src}
              </span>
            )
          })}
            ...
            return (
              ...
              >
                <div className="trace-step-head">
                  ...
                  {s.source && (
                    <span className={`trace-source trace-src-${classifySource(s.source)}`}>{s.source}</span>
                  )}
